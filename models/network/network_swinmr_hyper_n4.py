...
class HyperMlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = PHMLinear(n=get_phn(in_features, hidden_features), in_features=in_features, out_features=hidden_features, cuda=True)
        self.act = act_layer()
        self.fc2 = PHMLinear(n=get_phn(hidden_features, out_features), in_features=hidden_features, out_features=out_features, cuda=True)
...
class HyperWindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = PHMLinear(n=get_phn(dim, dim * 3), in_features=dim, out_features=dim * 3, cuda=True)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = PHMLinear(n=get_phn(dim, dim), in_features=dim, out_features=dim, cuda=True)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = HyperWindowAttention(dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
        qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = HyperMlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if resi_connection == '1conv':
            self.conv = PHConv(get_phn(dim, dim), dim, dim, 3, 1, 1)
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv = nn.Sequential(PHConv(get_phn(dim, dim // 4), dim, dim // 4, 3, 1, 1),
                                      nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                      PHConv(get_phn(dim // 4, dim // 4), dim // 4, dim // 4, 1, 1, 0),
                                      nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                      PHConv(get_phn(dim // 4, dim), dim // 4, dim, 3, 1, 1))
...
class SwinHyperIR(nn.Module):
    def __init__(self, img_size=64, patch_size=1, in_chans=1, out_chans=1,
        ...
        if depths is None:
            depths = [6, 6, 6, 6]
        self.num_in_ch = num_in_ch = in_chans  # 输入通道数
        self.num_out_ch = num_out_ch = out_chans
        num_feat = 64
        ...
        # 构建 Residual Swin Transformer Blocks（RSTB），传入内部层 ph_n
        ...
        if resi_connection == '1conv':
            self.conv_after_body = PHConv(get_phn(embed_dim, embed_dim), embed_dim, embed_dim, 3, 1, 1)
        elif resi_connection == '3conv':
            self.conv_after_body = nn.Sequential(
                PHConv(get_phn(embed_dim, embed_dim // 4), embed_dim, embed_dim // 4, 3, 1, 1),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                PHConv(get_phn(embed_dim // 4, embed_dim // 4), embed_dim // 4, embed_dim // 4, 1, 1, 0),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                PHConv(get_phn(embed_dim // 4, embed_dim), embed_dim // 4, embed_dim, 3, 1, 1)
            )
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_last = PHConv(get_phn(num_feat, num_out_ch), num_feat, num_out_ch, 3, 1, 1)
            ...
            self.conv_last = PHConv(get_phn(embed_dim, num_out_ch), embed_dim, num_out_ch, 3, 1, 1)
    ...
    def forward_features(self, x):
        x_size = (x.shape[2], x.shape[
            3])  # x (4,180,96,96) (batch_size_in_each_GPU, embedding_channel180, H (random-crop 96 in traning and 256 in testing), W) x_size (96,96)
        ...
        x = self.patch_unembed(x,
                               x_size)  # x (4,180,96,96) (batch_size_in_each_GPU, embedding_channel180, H (random-crop 96 in traning and 256 in testing), W) x_size (96,96)

        return x

    def forward(self, x):
        H, W = x.shape[
               2:]  # x (4,1,96,96) (batch_size_in_each_GPU, input_image_channel, H (random-crop 96 in traning and 256 in testing), W)
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x_first = self.conv_first(
                x)  # x_first (4,180,96,96) (batch_size_in_each_GPU, embedding_channel180, H (random-crop 96 in traning and 256 in testing), W)
...
def get_phn(in_channels, out_channels):

    if in_channels >= 4 and out_channels >= 4 and (in_channels % 4 == 0) and (out_channels % 4 == 0):
        return 4
    else:
        return 2
