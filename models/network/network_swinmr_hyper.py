...
class HyperMlp(nn.Module):
    def __init__(self, ph_n, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = PHMLinear(n=ph_n, in_features=in_features, out_features=hidden_features, cuda=True)
        self.act = act_layer()
        self.fc2 = PHMLinear(n=ph_n, in_features=hidden_features, out_features=out_features, cuda=True)
...
class HyperWindowAttention(nn.Module):
    ...
    def __init__(self, ph_n, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = PHMLinear(n=ph_n, in_features=dim, out_features=dim * 3, cuda=True)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = PHMLinear(n=ph_n, in_features=dim, out_features=dim, cuda=True)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, ph_n, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = HyperWindowAttention(ph_n, dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
        qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)

        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = HyperMlp(ph_n, in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, ph_n, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(ph_n, dim=dim, input_resolution=input_resolution,
...
class RSTB(nn.Module):
    ...
    def __init__(self, ph_n, dim, input_resolution, depth, num_heads, window_size,
        ...
        self.residual_group = BasicLayer(ph_n, dim=dim,
        ...
        if resi_connection == '1conv':
            self.conv = PHConv(ph_n, dim, dim, 3, 1, 1)
        elif resi_connection == '3conv':
            # to save parameters and memory
            self.conv = nn.Sequential(PHConv(ph_n, dim, dim // 4, 3, 1, 1),
                                      nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                      PHConv(ph_n, dim // 4, dim // 4, 1, 1, 0),
                                      nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                      PHConv(ph_n, dim // 4, dim, 3, 1, 1))
...
class SwinHyperIR(nn.Module):
    ...
    def __init__(self, ph_n=2, img_size=64, patch_size=1, in_chans=1, out_chans=1,
                 ...
                 **kwargs):
        super(SwinHyperIR, self).__init__()
        self.num_in_ch = num_in_ch = in_chans  # 1
        self.num_out_ch = num_out_ch = out_chans  # 1
        num_feat = 64
        ...
        print(f"Type of ph_n: {type(self.ph_n)}, Value: {self.ph_n}")
        ...
        try:
            ...
            raise ValueError(f"Cannot convert {img_size} to integer")

        #####################################################################################################
        ################################### 1, shallow feature extraction ###################################
        # self.conv_first = nn.Conv2d(num_in_ch, embed_dim, 3, 1, 1)  # in 1 out 180
        if num_in_ch == 1 or num_in_ch == 3:
            n = 1
        else:
            n = ph_n

        self.conv_first = PHConv(
            n=n,
            in_features=num_in_ch,
        ...
        if resi_connection == '1conv':
            self.conv_after_body = PHConv(ph_n, embed_dim, embed_dim, 3, 1, 1)
        elif resi_connection == '3conv':
            # to save parameters and memory
            ...
            self.conv_after_body = nn.Sequential(PHConv(ph_n, embed_dim, embed_dim // 4, 3, 1, 1),
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 PHConv(ph_n, embed_dim // 4, embed_dim // 4, 1, 1, 0),
                                                 nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                                 PHConv(ph_n, embed_dim // 4, embed_dim, 3, 1, 1))
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.conv_last = PHConv(ph_n, num_feat, num_out_ch, 3, 1, 1)
            ...
            self.conv_last = PHConv(ph_n, embed_dim, num_out_ch, 3, 1, 1)
    ...
    def flops(self):
        ...
        flops += H * W * self.embed_dim * self.num_out_ch * 9
        return flops

    def params(self):
        params = 0
        params += self.num_in_ch * self.embed_dim * 9
        params += self.patch_embed.params()
        for i, layer in enumerate(self.layers):
            params += layer.params()
        params += self.embed_dim * self.embed_dim * 9
        params += self.embed_dim * self.num_out_ch * 9
        return params
...
if __name__ == '__main__':
    ...
    gpu_mem_used_1 = torch.cuda.max_memory_allocated()
    ...
    config['patch_size'] = 1
    config['upscale'] = 1
    config['in_chans'] = channel
    config['img_size'] = height
    config['window_size'] = 8
    config['img_range'] = 1.0
    ...
    model = SwinHyperIR(**config).to(device)
    ...
    # with torch.no_grad():
    #     x = model(x)
    #     gpu_mem_used_2 = torch.cuda.max_memory_allocated()
    ...
    # print('Memory used: {}MiB'.format(round((gpu_mem_used_2 - gpu_mem_used_1) / 1024 / 1024, 3)))
