...
try:
    ...
    import dense_layer, layers, up_or_down_sampling
...
class ResnetBlockBigGANpp_Adagn_update(nn.Module):
    def __init__(self, act, in_ch, out_ch=None, temb_dim=None, zemb_dim=None, up=False, down=False,
        ...
        if self.up:
            self.up_block = Upsample(in_channels=in_ch, with_conv=True)
        elif self.down:
            self.down_block = Downsample(in_channels=in_ch, with_conv=True)
